...
def load_agg_facts():
    ...
    update_tgt_aggregate_sales = f"""
                ...
                ROW_UPDT_TMS = LOCALTIMESTAMP
                FROM BOSS_DB.TEMP.TMP_SALES AS TMP_SALES
                WHERE (TGT.PDT_KY = TMP_SALES.PDT_KY AND TGT.LOCN_KY = TMP_SALES.LOCN_KY 
    ...
    load_tgt_aggregate_sales = f""" INSERT INTO BOSS_DB.TARGET.{table} (
        ...
        TOTAL_DSCNT,
        ROW_INSRT_TMS,
        ROW_UPDT_TMS) SELECT S.PDT_KY, S.LOCN_KY, D.MONTH_KY,
                        SUM(S.QTY), SUM(AMT), SUM(DSCNT), LOCALTIMESTAMP, LOCALTIMESTAMP FROM BOSS_DB.TEMP.TMP_SALES S
                        INNER JOIN BOSS_DB.TARGET.D_BOSS_TIME_DAY_T D ON D.DAY_KY = S.DT_KY
                        WHERE  (S.PDT_KY, s.LOCN_KY, D.MONTH_KY) NOT IN 
                        (SELECT DISTINCT  PDT_KY, LOCN_KY,MONTH_KY FROM BOSS_DB.TARGET.{table})
                        GROUP BY 1,2,3;"""
